...
public class ChatDatabaseService {
    ...
    private String extractProductName(String question) {
        ...
        Pattern pattern = Pattern.compile("(?:tìm|search|thông tin)\\s+(?:về\\s+)?([\\w\\s]+?)(?:\\?|$)",
    ...
    public String createAIPrompt(String question, DatabaseResponse dbResponse) {
        ...
        if (dbResponse.getDataType().equals("product_search") || dbResponse.getDataType().equals("product_list")) {
            ...
            for (Product product : products) {
                prompt.append("- ").append(product.getName())
                        ...
                        .append(" - Thương hiệu: ")
                        .append(product.getBrand() != null ? product.getBrand().getName() : "N/A")
                        .append(" - Danh mục: ")
        ...
        prompt.append(
                "Hãy trả lời dựa trên thông tin database trên. Nếu câu hỏi không liên quan đến thông tin này, hãy trả lời chung chung về Itel Shop.");
